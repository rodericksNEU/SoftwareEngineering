...
export declare const mockDeep: <T>(mockImplementation?: DeepPartial<T> | undefined) => { [K in keyof T]: T[K] extends (...args: infer A) => infer B ? CalledWithMock<B, A> : MockProxy<T[K]>; } & T;
declare const mock: <T>(mockImplementation?: DeepPartial<T>, opts?: MockOpts | undefined) => { [K in keyof T]: T[K] extends (...args: infer A) => infer B ? CalledWithMock<B, A> : MockProxy<T[K]>; } & T;
