...
describe('Matchers', () => {
    ...
    describe('captor', () => {
        ...
        test('can capture arg with other matchers', () => {
            doSomething(fn, 1);
            const argCaptor = Matchers_1.captor();
            expect(fn).toHaveBeenCalledWith(argCaptor, Matchers_1.any(), Matchers_1.any());
        ...
        test('stores all values', () => {
            ...
            doSomething(fn, 3);
            const argCaptor = Matchers_1.captor();
            expect(fn).toHaveBeenNthCalledWith(1, argCaptor, Matchers_1.any(), Matchers_1.any());
            expect(fn).toHaveBeenNthCalledWith(2, argCaptor, Matchers_1.any(), Matchers_1.any());
            expect(fn).toHaveBeenNthCalledWith(3, argCaptor, Matchers_1.any(), Matchers_1.any());
    ...
    describe('matches function', () => {
        test('expects passes for when it returns true', () => {
            const fn = jest.fn();
            fn(1);
            expect(fn).toHaveBeenCalledWith(Matchers_1.matches(val => val === 1));
        });
        test('expects with not passes for when it returns false', () => {
            const fn = jest.fn();
            fn(1);
            expect(fn).not.toHaveBeenCalledWith(Matchers_1.matches(val => val === 2));
        });
    });
});
